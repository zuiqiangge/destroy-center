...
public class ApplyDao extends BaseDao<Apply, Integer> implements IApplyDao {
	...
	public List<Apply> getListbyCondition(Apply apply) {
		String hql = "from "+Apply.class.getName()+" where 1=1";
		
		if(null!=apply.getSubmitUserId()&&apply.getSubmitUserId()!=0&&apply.getSubmitUserId()!=-1)
			hql+=" and submitUserId='"+apply.getSubmitUserId()+"'";
		if(null!=apply.getContactUnit()&&!(StringUtils.isEmpty(apply.getContactUnit())))
			hql+=" and contactUnit='"+apply.getContactUnit()+"'";
		if(null!=apply.getContacts()&&!(StringUtils.isEmpty(apply.getContacts())))
			hql+=" and contacts='"+apply.getContacts()+"'";
		if(null!=apply.getTelephone()&&!(StringUtils.isEmpty(apply.getTelephone())))
			hql+=" and telephone='"+apply.getTelephone()+"'";
		if(null!=apply.getAddress()&&!(StringUtils.isEmpty(apply.getAddress())))
			hql+="and address='"+apply.getAddress()+"'";
		if(null!=apply.getSendType())
			hql+=" and sendType='"+apply.getSendType()+"'";
		if(null!=apply.getNowDestroy())
			hql+=" and nowDestroy='"+apply.getNowDestroy()+"'";
		List<Apply> applies = new ArrayList<Apply>();
		applies = getHibernateTemplate().find(hql);
		return applies;
	}

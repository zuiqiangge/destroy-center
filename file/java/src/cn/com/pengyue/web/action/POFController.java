...
public class POFController {
	...
	public String toListFilesWei(HttpSession session, Model model,
		...
		if (null == request.getParameter("bigType")){
			...
			try {
				contactUnit = new String(contactUnit.getBytes("iso8859-1"),"utf-8");
	 ...
	 * @param request
	 */
	public List<PlaceOnFile> getFileYiData(PlaceOnFile file,HttpServletResponse response,
			@RequestParam("start") int start,@RequestParam("length") int length){
		return null;//pOFService.getFileYiData(file);
	}
